...
class Calculator {
     ...
     */
    void evaluate() {
        final ArrayDeque<Long> rpnHelper = new ArrayDeque<>();

        while (!rpn.isEmpty()) {
            if (rpn.peekFirst().matches("[+-]?\\d+")) {
                rpnHelper.offer(Long.parseLong(rpn.poll()));
                ...
                final long a = rpnHelper.pollLast();
                final long b = rpnHelper.pollLast();
                ...
                switch (rpn.peekFirst()) {
                    ...
                    case "^":
                        rpnHelper.offer((long) Math.pow(b, a));
                        break;
                }
...
class ErrorCheck extends Calculator {
    ...
    static void check(final String tokens) {
        ...
        if (!varVal.matches("[+-]?\\s*[a-zA-Z0-9]+")) {
            ...
            vars.put(varName, Long.parseLong(varVal.replaceAll("\\s*", "")));
            isAssignment = true;
            ...
            if (isUnary) {
                if (vv.matches("[+-]?\\d+")) {
                    vars.put(varName, Long.parseLong(varVal));
                    isAssignment = true;
                } else {
                    String ss = vv.substring(1);
                    if (vars.containsKey(ss)) {
                        vars.put(varName, vv.charAt(0) == '-' ? -1 * vars.get(ss) : vars.get(ss));
                        isAssignment = true;
                    } else {
                        result = "Unknown variable";
                        isError = true;
